...
// import { makePoly2DStyle, Poly2D } from './poly2d'
import { blendColor, Context2D, encodeControlColor, getColorById } from './util'

// type Shape = Poly2D | Point2D

const _DEFAULT_VIEW_EDGE_STYLE = makeEdge2DStyle({ lineWidth: 4 })
const _DEFAULT_VIEW_POINT_STYLE = makePoint2DStyle({ radius: 8 })
const _DEFAULT_VIEW_HIGH_POINT_STYLE = makePoint2DStyle({ radius: 12 })
const _DEFAULT_CONTROL_EDGE_STYLE = makeEdge2DStyle({ lineWidth: 10 })
const _DEFAULT_CONTROL_POINT_STYLE = makePoint2DStyle({ radius: 12 })
...
export class Polygon2D extends Label2D {
 ...
 /** for updateshapes */
  ...
  public _updateShapes (_shapes: ShapeType[]): void {
    // if (this._label !== null) {
